...
async def generate_ai_narrative(
    ...
    Returns (narrative_text, used_claude: bool). used_claude=True when Claude was used.
    """
    # Используем Claude, если ключ задан
    api_key = (getattr(settings, "ANTHROPIC_API_KEY", "") or "").strip()
    if not api_key:
        log.info(
            "AI narrative skipped: ANTHROPIC_API_KEY not set (LLM_ENABLED=%s)",
            getattr(settings, "LLM_ENABLED", False),
        )
        return _template_narrative(tenant, assessment, stats, top_gaps), False
    ...
    try:
        ...
        log.error("Claude API error in generate_ai_narrative: %s", e)
        # Не выводим детали ошибки (биллинг, ключи) в отчёт — только общее сообщение
        return _template_narrative(tenant, assessment, stats, top_gaps) + (
            "\n\n[AI summary was not available for this report. A template summary is shown above.]"
        ), False
...
async def generate_executive_summary(
    ...
    # Полный контекст для Claude: если включён AI и есть ключ или LLM_ENABLED
    _llm_available = bool((getattr(settings, "ANTHROPIC_API_KEY", "") or "").strip()) or getattr(
        settings, "LLM_ENABLED", False
    )
    if include_ai and _llm_available:
        try:
    ...
    narrative, used_claude = await generate_ai_narrative(
    ...
    if used_claude:
        story.append(Paragraph(
            "<i>This summary was generated by Claude AI from your questionnaire answers, evidence, and agent data.</i>",
            disclaimer_style,
        ))
